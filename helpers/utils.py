...
def saveCheckpoint(
    ...
    try:
        with open(f"{dialogPath}/CheckPoint.json", "r") as f:
            ...
            if lastMessageID:
                dialog[0] = lastMessageID
            if messageCounter:
                dialog[1] = messageCounter
            if fileCounter:
                dialog[2] = fileCounter
            if flagOfGetDialogInfo:
                dialog[3] = flagOfGetDialogInfo
            if timeStart:
                dialog[4] = time.perf_counter() - timeStart
